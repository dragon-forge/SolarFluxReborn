package com.mrdimka.solarfluxreborn.core;
...
public class SFRSerializedTransformer implements IClassTransformer, Serializable
{
	...
	static
	{
		...
		for(int i = 0; i < data.length; ++i) if(data[i] == '.') data[i] = '/';
		ser = new String(data);
		SerializableClass = ser;
		
//		SFRLog.info("\"%s\" application for class * is \"%s\"", Serializable.class.getName(), ser);
	}
...
//		if((!name.contains(".") || name.startsWith("net.minecraft") || name.startsWith("mcp") || name.startsWith("ibxm")) && cls.interfaces.size() == 0)
//		{
