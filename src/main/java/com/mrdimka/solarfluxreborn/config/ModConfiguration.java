...
public class ModConfiguration {
	public static final List<TierConfiguration> DEFAULT_TIER_CONFIGURATIONS = ImmutableList.of(
			...
			new TierConfiguration(8192, 64 * MetricUnits.KILO, 64 * MetricUnits.MEGA),
			new TierConfiguration(32768, 256 * MetricUnits.KILO, 128 * MetricUnits.MEGA));
	...
	public static void initialize(File pConfigFile, File versionFile)
	{
		...
		if(cfg == null) cfg = new Configuration(pConfigFile);
	...
	private static void saveUpdateVersionDat(File version)
	{
		try
		{
			if(!version.isFile()) version.createNewFile();
	...
	public static void updateNotification(boolean willNotify)
	{
		...
		saveUpdateVersionDat(new File(SolarFluxRebornMod.cfgFolder, "version.dat"));
	}

	private static void loadConfiguration()
	{
		if(cfg == null) initialize(new File(SolarFluxRebornMod.cfgFolder, "main.cfg"), new File(SolarFluxRebornMod.cfgFolder, "version.dat"));
		autoBalanceEnergy = cfg.getBoolean("BalanceEnergy", Configuration.CATEGORY_GENERAL, true,
				"Neighbor solar panels share their energy if set to true.");
		keepEnergyWhenDismantled = cfg.getBoolean("KeepEnergyWhenDismantled",
		...
		keepInventoryWhenDismantled = cfg.getBoolean("KeepInventoryWhenDismantled",
				Configuration.CATEGORY_GENERAL, true,
				"Whether or not the solar panels keep their internal inventory when dismantled with a wrench.");
		rainGenerationFactor = cfg.getFloat("RainProductionFactor", Configuration.CATEGORY_GENERAL, 0.4f, 0,
				1, "Factor used to reduce the energy generation during rainy weather.");
		thunderGenerationFactor = cfg.getFloat("ThunderProductionFactor", Configuration.CATEGORY_GENERAL,
		...
		BlockCable320.TRANSFER_RATE = cfg.getFloat("cable t1", "cables", 320F, 1F, 1000000F, "How much RF can first cable transfer.");
		BlockCable3200.TRANSFER_RATE = cfg.getFloat("cable t2", "cables", 3200F, 1F, 1000000F, "How much RF can second cable transfer.");
		BlockCable320000.TRANSFER_RATE = cfg.getFloat("cable t3", "cables", 320000F, 1F, 1000000F, "How much RF can last cable transfer.");
	...
	private static void loadUpgradesConfiguration() {
		// Efficiency upgrade
		mEfficiencyUpgradeActive = cfg.getBoolean("EfficiencyUpgradeActive", UPGRADE_CATEGORY, true,
				"Whether or not efficiency upgrades should be added to the game.");
		mEfficiencyUpgradeIncrease = cfg.getFloat("EfficiencyUpgradeIncrease", UPGRADE_CATEGORY, 0.05F,
				0.01F, 10, "Factor by which the energy production is increased per upgrade.");
		mEfficiencyUpgradeReturnsToScale = cfg.getFloat("EfficiencyUpgradeReturnsToScale", UPGRADE_CATEGORY,
				0.9F, 0.1F, 2,
				"Returns to scale. How does the efficiency scales as you add more upgrades. 1 is linear. Below 1 reduces the efficiency as you add upgrades, Above 1 does the opposite.");
		mEfficiencyUpgradeMax = cfg.getInt("EfficiencyUpgradeMax", UPGRADE_CATEGORY, 8, 1, 256,
				"Maximum number of efficiency upgrade that can be added to a single solar panel.");

		// Low light upgrade
		mLowLightUpgradeActive = cfg.getBoolean("LowLightUpgradeActive", UPGRADE_CATEGORY, true,
				"Whether or not low light upgrades should be added to the game.");
		mLowLightUpgradeMax = cfg.getInt("LowLightUpgradeMax", UPGRADE_CATEGORY, 8, 1, 256,
				"Maximum number of low light upgrade that can be added to a single solar panel.");

		// Traversal upgrade
		mTraversalUpgradeActive = cfg.getBoolean("TraversalUpgradeActive", UPGRADE_CATEGORY, true,
				"Whether or not traversal upgrades should be added to the game.");
		mTraversalUpgradeIncrease = cfg.getInt("TraversalUpgradeIncrease", UPGRADE_CATEGORY, 1, 1, 10,
				"How many extra machines can be traversed per extra upgrade.");
		mTraversalUpgradeUpdateRate = cfg.getInt("TraversalUpgradeUpdateRate", UPGRADE_CATEGORY, 25, 1,
				20 * 60, "Update rate of traversal. Increase to reduce lag. But machines will be discovered slower.");
		mTraversalUpgradeMax = cfg.getInt("TraversalUpgradeMax", UPGRADE_CATEGORY, 64, 1, 256,
				"Maximum number of traversal upgrade that can be added to a single solar panel.");

		// Transfer upgrade
		mTransferRateUpgradeActive = cfg.getBoolean("TransferRateUpgradeActive", UPGRADE_CATEGORY, true,
				"Whether or not transfer rate upgrades should be added to the game.");
		mTransferRateUpgradeIncrease = cfg.getFloat("TransferRateUpgradeIncrease", UPGRADE_CATEGORY, 0.1F,
				0.01F, 10, "Factor by which the transfer rate is increased per upgrade.");
		mTransferRateUpgradeReturnsToScale = cfg.getFloat("TransferRateUpgradeReturnsToScale",
				UPGRADE_CATEGORY, 0.9F, 0.1F, 2,
				"Returns to scale. How does the transfer rate scales as you add more upgrades. 1 is linear. Below 1 reduces the efficiency as you add upgrades, Above 1 does the opposite.");
		mTransferRateUpgradeMax = cfg.getInt("TransferRateUpgradeMax", UPGRADE_CATEGORY, 8, 1, 256,
				"Maximum number of transfer rate upgrade that can be added to a single solar panel.");

		// Capacity upgrade
		mCapacityUpgradeActive = cfg.getBoolean("CapacityUpgradeActive", UPGRADE_CATEGORY, true,
				"Whether or not capacity upgrades should be added to the game.");
		mCapacityUpgradeIncrease = cfg.getFloat("CapacityUpgradeIncrease", UPGRADE_CATEGORY, 0.1F, 0.01F, 10,
				"Factor by which the capacity is increased per upgrade.");
		mCapacityUpgradeReturnsToScale = cfg.getFloat("CapacityUpgradeReturnsToScale", UPGRADE_CATEGORY, 1,
				0.1F, 2,
				"Returns to scale. How does the transfer rate scales as you add more upgrades. 1 is linear. Below 1 reduces the efficiency as you add upgrades, Above 1 does the opposite.");
		mCapacityUpgradeMax = cfg.getInt("CapacityUpgradeMax", UPGRADE_CATEGORY, 16, 1, 256,
				"Maximum number of capacity upgrade that can be added to a single solar panel.");

		// Furnace upgrade
		mFurnaceUpgradeActive = cfg.getBoolean("FurnaceUpgradeActive", UPGRADE_CATEGORY, true,
				"Whether or not furnace upgrades should be added to the game.");
		mFurnaceUpgradeHeatingConsumption = cfg.getInt("FurnaceUpgradeHeatingConsumption", UPGRADE_CATEGORY,
				8, 1, 64000, "Amount of RF per tick used to heat up a furnace.");
	}
	...
	private static void loadTierConfigurations()
	{
		...
		for(int tier = 0;; ++tier)
		{
			...
			else mTierConfigurations.add(config);
		}
	}
	...
	private static TierConfiguration loadTierConfiguration(int pTierNumber)
	{
		...
		if (active)
		{
			...
			if(pTierNumber < DEFAULT_TIER_CONFIGURATIONS.size()) defaultConfig = DEFAULT_TIER_CONFIGURATIONS.get(pTierNumber);
			else
			{
			...
			return new TierConfiguration(
					cfg.getInt("MaximumEnergyGeneration", category,
							defaultConfig.getMaximumEnergyGeneration(), 1, 2 * MetricUnits.GIGA,
							"Maximum amount of RF generated per tick."),
					cfg.getInt("MaximumEnergyTransfer", category, defaultConfig.getMaximumEnergyTransfer(),
							1, 2 * MetricUnits.GIGA, "Maximum amount of RF transferred per tick."),
					cfg.getInt("Capacity", category, defaultConfig.getCapacity(), 1, 2 * MetricUnits.GIGA,
