		...
		extends UpgradeItem
{
	...
	public boolean canInstall(ISolarPanelTile tile, ItemStack stack, SimpleInventory upgradeInv)
	{
		BlockPos pos;
		TileEntity t;
		return stack.hasTag() && stack.getTag().contains("Pos", NBT.TAG_LONG) && stack.getTag().contains("Face", NBT.TAG_BYTE) && (!stack.getTag().contains("Dim", NBT.TAG_STRING) || tile.level().dimension().location().toString().equals(stack.getTag().getString("Dim"))) && tile.pos().distSqr(pos = BlockPos.of(stack.getTag().getLong("Pos"))) <= 256D && (t = tile.level().getBlockEntity(pos)) != null && t.getCapability(CapabilityEnergy.ENERGY, Direction.values()[stack.getTag().getByte("Face")]).isPresent();
	}
	...
	{
		return new Object[] { Math.round((float) Math.sqrt(256)) };
	}
}