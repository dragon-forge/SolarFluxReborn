		...
		extends UpgradeItem
{
	...
	public void update(SolarPanelTile tile, ItemStack stack, int amount)
	{
		...
		if(t instanceof AbstractFurnaceTileEntity)
		{
			...
			AbstractCookingRecipe irecipe = tf.getWorld().getRecipeManager().getRecipe(tf.recipeType, tf, tf.getWorld()).orElse(null);

			if(tf.burnTime < 1 && irecipe != null && canSmelt(tf, irecipe) && tile.energy >= 1000)
			{
				tf.recipesUsed = tf.burnTime = irecipe.getCookTime();
				tile.energy -= 1000;
			}
		}
	}
	...
	{
		if(!f.getStackInSlot(0).isEmpty() && recipeIn != null)
		{
			...
			{
				ItemStack itemstack1 = f.getStackInSlot(2);
